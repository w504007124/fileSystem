...
public class FileUtil implements Serializable{
    ...
    public static void zipBytes(List<byte[]> srcfiles, File zipfile,List<String> fileNames) {
        byte[] buf = new byte[1024];
        try {
            ZipOutputStream out = new ZipOutputStream(new FileOutputStream(zipfile));
            for (int i = 0; i < srcfiles.size(); i++) {
                InputStream  sbs =  new ByteArrayInputStream(srcfiles.get(i));
                out.putNextEntry(new ZipEntry(fileNames.get(i)));
                int len;
                while ((len = sbs.read(buf)) > 0) {
                    out.write(buf, 0, len);
                }
                out.closeEntry();
                sbs.close();
